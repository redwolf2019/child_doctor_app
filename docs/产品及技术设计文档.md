...
- App 不锁死系统方向。竖屏时只显示转横提示，横屏后才显示检查室。这样才能可靠识别“设备正被竖着拿”。
...
AndroidManifest 和 `SystemChrome.setPreferredOrientations` 都不应锁定为 landscape。锁定后 Flutter 得到的窗口通常一直是横屏尺寸，门禁无法判断成人正把设备竖着拿。
...
| 类型 | 依赖 | 用途 | 约束 |
| --- | --- | --- | --- |
| 运行时 | Flutter SDK | UI、生命周期、方向和 Android 壳 | 初始化时锁定稳定版并提交工具链版本文件 |
| 运行时 | `lottie` | 播放本地 `scan.json` | 版本写死在 `pubspec.yaml`，升级需回归正式动画 |
| 运行时 | `audioplayers` | 播放和停止本地 MP3 | 只用本地 `AssetSource`，不传 URL |
| 开发 | `flutter_lints` | 静态检查 | `flutter analyze` 无错误 |
...
| App 显示名称 | 肚子扫描仪 |
| `minSdk` | 26 |
| `targetSdk` / `compileSdk` | 使用已锁定 Flutter 稳定版生成并验证的值，实际数字提交在 Gradle 配置中 |
| 屏幕方向 | 不设置 `android:screenOrientation="landscape"` |
...
| Flutter 稳定版和依赖精确版本 | 初始化工程时 | 无法复现构建；必须写入工程，不能长期悬空 |

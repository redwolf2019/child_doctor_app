...
术语以 [CONTEXT.md](../CONTEXT.md) 为准。平台范围见 [ADR 0001](./adr/0001-flutter-android-apk-only.md)，动画选型见 [ADR 0002](./adr/0002-lottie-for-scan.md)，扫描关键帧位图见 [ADR 0004](./adr/0004-scan-keyframe-bitmaps.md)。若本文与 ADR 冲突，以状态为“已接受”的 ADR 为准；产品文案和交互以本文为准。
...
检查室是亮色卡通医院。卡通 X 光机和卡通小孩已经在场，卡通小孩略偏左，屏幕下方只有一个主按钮“开始扫描”。不要做标题、玩法说明或设置入口。
...
建议分镜如下。它是首版素材的制作基线，正式交付时可在总时长范围内微调。第 0 帧必须和检查室静图同一全景；随后镜头推近肚子，不要硬切：

| 时间 | 画面 | 声音 |
| --- | --- | --- |
| 0.0～1.0 s | 检查室全景，卡通小孩略偏左，机器亮起，扫描线出现 | 扫描音效起 |
| 1.0～4.5 s | 扫描线扫过卡通小孩，身体保持卡通质感 | 扫描音效持续 |
| 4.5～7.0 s | 肚子逐渐半透明，露出简化肠道，镜头开始推近 | 扫描音效持续 |
| 约 7.0 s | 切到肚子近景，圆滚滚的虫子清楚可见 | 虫子蠕动音效播放一次 |
| 7.0～11.5 s | 近景停留，虫子可数，右侧留空给洗手提示板 | 扫描音效持续 |
| 11.5～12.0 s | 扫描线熄灭，停在近景末帧 | 扫描音效淡出或直接停止 |

虫子要“有点恶心但不恐怖”：圆钝轮廓、数量可数、不出现尖牙、伤口、血液、排泄物或写实皮肤纹理。肠道不标名字，不使用真实影像。

### 7.5 洗手提示（`result`）

结果页保留 Lottie 末帧（肚子近景），由 Flutter 在舞台右侧叠加浅色提示板。提示板不超过 16:9 舞台宽度的 38%，位于安全区内，文案和“再看一次”都左对齐；虫子留在画面中左，不让提示板盖住。文案不能烧进 Lottie。固定原文是：

> 不洗手就吃东西，肚子里会长虫子。快去洗手！

文案最多排四行，左对齐。进入 `result` 时 Lottie 控制器停在完成位置，配音播放一次且不循环。提示板内文案下方显示“再看一次”按钮。点击后先停配音，再把同一个控制器归零并从第 0 帧播放。
...
| 文件路径 | 格式和规格 | 用途 | 验收 |
| --- | --- | --- | --- |
| `assets/images/exam_room.webp` | 1920 × 1080，sRGB，不含按钮和文字 | 检查室背景；卡通小孩略偏左 | `BoxFit.contain` 下四边无关键内容被裁 |
| `assets/lottie/scan.json` | 1920 × 1080 画布，30 fps，约 360 帧，不含文字 | 扫描全过程：交叉淡化三张关键帧 + 矢量扫描线 | Lottie Flutter 真机可播；第 0 帧与检查室静图对齐；包内位图无缺图 |
| `assets/lottie/scan_ready.webp` | 1920 × 1080，sRGB，无文字 | 扫描第 0 帧，与 `exam_room.webp` 同一全景 | 与检查室静图构图一致 |
| `assets/lottie/scan_mid.webp` | 1920 × 1080，sRGB，无文字 | 扫描中段：半透明肚子和肠道 | 肠道可辨，无虫子或仅隐约 |
| `assets/lottie/scan_closeup.webp` | 1920 × 1080，sRGB，无文字 | 扫描末帧：肚子近景和可数虫子，右侧留空 | 虫子圆钝可数；右侧约三分之一为墙面 |
...
`scan.json` 引用同目录三张包内 WebP，不引用远程 URL，也不使用 `../` 跳出 `assets/lottie/`。这三张图必须写进 `pubspec.yaml`。扫描线仍用矢量层。动画制作工具的预览不算验收，以 Flutter 真机播放为准。见 [ADR 0004](./adr/0004-scan-keyframe-bitmaps.md)。
...
| 占位界面颜色 | 舞台外 `#16324F`；主按钮 `#FFB703`；按钮文字 `#3A2A00`；结果板 `#FFF8E7`、96% 不透明度；正文 `#203040` | 正式视觉稿确定后统一替换 design tokens 和 Golden 图 |
